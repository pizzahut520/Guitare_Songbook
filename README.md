...
通过 Cloudflare Access 登录后可在 `/add/` 切换到“手动添加歌词”。此模式完全在浏览器本地解析，不调用 AI；填写元数据和全文后选择“解析并预览结构”，再在同一套编辑器中补全级数和弦。空和弦只允许存在于浏览器 draft，必须填入合法级数后才能确认发布。

全文可使用以下独占一行的结构标记：`[主歌]`、`[预副歌]`、`[副歌]`、`[Bridge]` / `[桥段]`、`[尾段]`、`[其他]`。A/B 共享和弦使用显式标记；可选组名允许不相邻配对：
...
对应行会成为一条共享 `chords` 与两套 `lyric_sets` 的编辑项。没有组名时，A 后必须紧接同结构的 B。若真实歌词以 `[` 开头，可写成 `\[歌词]` 转义。标记解析不会猜测重复内容、和弦或 A/B。

A/B 是所有歌曲候选、手动录入和已发布曲谱都可使用的通用 Schema 能力，而非任何特定歌曲的例外。编辑器可添加/交换/删除歌词组、拆分为普通歌词段，并按 block ID 批量配对；所有操作保留 A 段的原始和弦空格。草稿不会写入 localStorage、URL、日志或数据库，刷新页面会丢失未提交内容。
